...
# Debian's default icecast.xml (from debconf, generated by the package's postinst) commonly
# ships with <sources>2</sources> — fine for a single dongle, too low once a second remote
# client (or a dongle with more than one voice channel) starts pushing. Raise it.
#
# Debian's default <burst-size> (65536 bytes) also assumes file/music streaming, where a
# fat initial buffer just means a faster start. For a low-bitrate live voice relay it means
# every new listener starts over a minute *behind* real time (and stays that far behind for
# the whole session) — confirmed in the field: listeners heard ~1-2 minutes of delay and
# repeated audio on reconnect. Shrink it so listeners join close to live.
if [ -f /etc/icecast2/icecast.xml ]; then
  $SUDO sed -i 's|<sources>[0-9]*</sources>|<sources>20</sources>|' /etc/icecast2/icecast.xml
  $SUDO sed -i 's|<burst-size>[0-9]*</burst-size>|<burst-size>8192</burst-size>|' /etc/icecast2/icecast.xml
  $SUDO sed -i 's|<queue-size>[0-9]*</queue-size>|<queue-size>131072</queue-size>|' /etc/icecast2/icecast.xml
fi
